...
setup_mpd() {
    run_loader "Creando configuracion de MPD" "mkdir -p ~/.config/mpd/playlists"

    run_loader "Habilitando servicio de MPD" "systemctl --user enable --now mpd"
}
...
run_network "Installing AUR Packages" "echo '$PASSWORD' | yay -S --needed --noconfirm --sudoloop fastfetch cmatrix cava ttf-iosevka otf-hermit-nerd gvfs dbus libdbusmenu-glib libdbusmenu-gtk3 gtk-layer-shell brave-bin zoxide eza fzf thefuck jq socat tmux nvm btop hyprshot bluez bluez-utils bluez-obex bluetuith python-gobject zsh-theme-powerlevel10k-git rmpc"
